...
- (instancetype)init {
    if (self = [super init]) {
        ...
        [[NSUserDefaults standardUserDefaults]setObject:[NSString stringWithFormat:@"%ld年",_year] forKey:@"year"];
        [[NSUserDefaults standardUserDefaults]setObject:[NSString stringWithFormat:@"%ld月",_month] forKey:@"month"];
        [[NSUserDefaults standardUserDefaults]setObject:[NSString stringWithFormat:@"%ld日",_day] forKey:@"month"];
...
- (NSArray *)setDayArr {
    ...
    for (NSInteger i = lastdayRange.length - components.weekday + 2; i <= lastdayRange.length; i++) {
        ...
        NSDictionary * dayDict = @{@"day":string,@"mark":@"lastOrNext"};
        [self.dayArray addObject:dayDict];
//        [self.dayArray addObject:string];
    }
    //本月的总天数
    for (NSInteger i = 1; i <= dayRange.length ; i++) {
        NSString * string = [NSString stringWithFormat:@"%ld",i];
        NSDictionary * dayDict = @{@"day":string,@"mark":@"theMonth"};
        [self.dayArray addObject:dayDict];
//        [self.dayArray addObject:string];
    }
    //下个月空出的天数
